...
function code($texte)
{
	...
	$texte = str_replace('xd', '<img src="../images/smiley/XD2.gif" alt="XD" title="XD" class="smiley" />', $texte);
	$texte = str_replace('&lt;3', '&hearts;', $texte);
...
/**
 * Remplace toutes les lettres avec accent par leur équivalent sans accent.
 * Tous les caractères spéciaux sont remplacés par un underscore "_"
 * @param String $chaine
 */
...
/**
 * Remplace tous les caractères spéciaux et lettres avec accents par un underscore "_"
 * @param string $chaine
 */
function noSpecialChar2($chaine){
 	$chaine = html_entity_decode($chaine);
    //  les accents
    $chaine=trim($chaine);
    $chaine= strtr($chaine,"ÀÁÂÃÄÅàáâãäåÒÓÔÕÖØòóôõöøÈÉÊËèéêëÇçÌÍÎÏìíîïÙÚÛÜùúûüÿÑñ","_____________________________________________________");
...
/**
 * Force the file $fic download
 * Must be the last instruction of a script
 * @param path $fic
 */
function forceDownload($fic){
