...
class Entite {
	...
	public $DB_table = '';
	// tableau d'équivalence nom variables class => nom champs SQL
	...
	public function loadFromDB($id_k,$id_v){
		$requete = 'SELECT * FROM '.$this->DB_table.' WHERE '.$id_k.' = '.$id_v;
	...
	public function enregistrer($toUpdate = null) {
		if(!is_null($this->id)) {
			...
			$requete = 'UPDATE '.$this->DB_table.' SET';
			$toSet = array();
			...
			//insert
			$requete = 'INSERT INTO '.$this->DB_table.' ';
			$column = array();
			$values = array();
			foreach ($this->DB_equiv as $key => $value) {
	...
	public function supprimer(){
		$requete = 'DELETE FROM '.$this->DB_table;
		$requete .= ' WHERE '.$this->DB_equiv['id'].' = '.$this->id;
		SQL::getInstance()->exec2($requete);
		return true;
	}
