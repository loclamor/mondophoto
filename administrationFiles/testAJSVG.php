...
//	include $emplacement;
	...
	$viewbox = '';
	//derni�re chose, on r�cup�re les attributs de l'�l�ment SVG pour conaitre le sens
...
<form action="<?php echo $urlAction->getUrl(); ?>" method="POST" >
	<div id="tabs">
		<ul>
			...
			<li id="2"><a href="#tabs-2">Selection des pays �tranger</a></li>
			<li id="3"><a href="#tabs-3">Nomage des villes</a></li>
			<li id="4"><a href="#tabs-4">Validation</a></li>
		</ul>
		<div id="tabs-1">
			<p>Cliquez sur le pays � ajouter</p>
			<input type="hidden" id="idPaysSVG" name="idPaysSVG" />
		</div>
		<div id="tabs-2">
			<p>Cliquez sur le pays �tranger � ajouter</p>
			<div id="dialog-modal-stranger" title="Informations sur le pays �tranger">
		...
		<div id="tabs-3">
			<p>Cliquez sur la ville � ajouter</p>
	...
	$(document).ready(function() {
		...
		$('#subButton').click(function(){
			if($('#idPaysSVG').attr('value') != '') {
				$('form').submit();
			/*	if($('.ville').length > 0){
					alert('Il reste des villes � placer.');
				}
				else {
					alert('ok');
				//	$('form').submit();
				...
				alert('Le pays de la carte n\'a pas �t� s�l�ction�.');
			}
		});
	});
</script>
